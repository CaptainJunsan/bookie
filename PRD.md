...
- **Fixed 2026-09-23, same day: relabeled the Immersion toggle.** Janico found it live — the family row already showed a "🗺️ Immersion" badge, but the only control for it (the "Child mode" toggle above) didn't mention Immersion anywhere, so it wasn't discoverable. Relabeled to "🗺️ Immersion mode" with copy describing both current effects; the underlying dual-purpose field itself is unchanged (see the point above).

## 22. Session log — School structure & mechanics, foundation pass (2026-09-23)

Built per §10, adopting §10.2's recommendation (a new top-level `schools` entity, not a club sub-type) and §10.3's ruling (schools can create learner profiles directly, with mandatory handover). Full plan and design rationale: see the approved plan this was built from, summarized below. **Scope was deliberately limited to the foundation** — schema, creation, enrolment, and handover/staff-invite flows. Deferred to a later pass, not started: class-scoped books/reading progress ("class reading mode," §10.4), the Reports tab (§10.5), and home-schooling's exportable PDF record (§10.6).

**Data model** (`supabase/schools_schema.sql`, applied to production `rnyatweedvzmeubqvjbo` as three migrations — `schools_feature_schema`, `school_staff_invites`, `school_staff_cross_visibility`): `schools`, `school_members` (admin/teacher roles, `class_id` nullable — a teacher of two classes gets two rows, no separate join table needed), `grades`, `classes` (with a short `join_code` for the class-join flow), `class_learners` (the single enrolment record covering both §10.3 routes — `family_member_id` set immediately when a parent links an existing child, or left null with a `handover_code` when a teacher creates the profile directly), and `school_staff_invites` (how an admin brings in a teacher who isn't already reachable through family/club channels). RLS follows the exact pattern already established for Clubs; `get_class_by_join_code`/`get_handover_preview`/`get_staff_invite_preview` are narrow, read-only SECURITY DEFINER RPCs rather than open table SELECTs, so a code's owner can be looked up without exposing every school/class to a full table scan. `claim_class_learner` mirrors `claim_child_member`'s shape but also creates the `family_members` row, since (unlike a family invite) there's no pre-existing row to attach to.

**UI:** `SchoolsPage.tsx` (list + create — no public directory, unlike Clubs, since joining a school always goes through an explicit code), `SchoolDetailPage.tsx` (tabs: Grades & Classes with the nested grade→class structure from the original ask, Staff with invite-by-code, Roster per class with "add a learner without a Bookie account yet" → handover code — every destructive action here has a tap-to-confirm step, matching `SettingsPage.tsx`'s existing pattern, since grade deletion cascades to its classes and their learners), and `JoinPage.tsx` at `/join` and `/join/:code` — a single universal code-entry point that resolves to a class join, a handover claim, or a staff invite, replacing the placeholder that pointed at `ExplorePage`. `ExplorePage.tsx`'s existing "Enter class code" button (already scaffolded before this session, unchanged) now leads somewhere real; added a "Manage your school" link alongside it. `OnboardingPage.tsx` gained a `returnTo` query param so a brand-new user who opens a join link with zero Bookie family gets bounced through onboarding and back to the link afterward.
